...
export const seedIssues = (project: Project, users: User[]): Promise<Issue[]> => {
...
const seedDatabase = async (): Promise<User> => {
  const users = await seedUsers();
  const project = await seedProject(users);
  const issues = await seedIssues(project, project.users);
